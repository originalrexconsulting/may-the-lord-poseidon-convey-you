...
without the timeout it would have sat for GitHub's six hours. The hang was found on
2026-09-20, after the Linux x86_64 job hung three times in one evening (`v2026.09.19.1`
twice, `v2026.09.19.2` once): `smoke-scie` piped `doctor`'s pty output into `grep -q`,
grep exits on its first match, the next write to the pipe fails, and Python's `pty.spawn`
then stops reading the child for good (it marks stdout dead and only selects on stdin), so
the child is never drained or reaped. It bit only where the output arrived in more than one
chunk; locally it lands in one and gets through. `smoke-scie` now captures the output to a
file and greps that. When one job times out, or a download to a runner drops mid-transfer (the Intel Mac job of
`v2026.09.14.8`),
...
state.json; all four runners in under two minutes) and `v2026.09.14.7` (the day's big one, three merged PRs: This day, Tours, On the air, Bookmarks, Stats and the taper's notes in the TUI; the scylla, sleestak, stealie and wall light show modes, 24 in all; the sleep timer with a fade, the phone remote on port 8402, and `poseidon play` / `poseidon remote` for a player without the TUI; all four runners and the publish job in about a minute and a half) and `v2026.09.14.8` (README screenshots for the day's menus and modes, the Sleestak warming up at real loudness, Charybdis drinking less often; the Intel Mac job failed in `make scie` when the download of its Python dropped mid-transfer, `httpx.RemoteProtocolError: Server disconnected without sending a response`, and was rerun on its own with `gh run rerun --job`, a minute; the publish job followed) and `v2026.09.15` (docs only: AUDIO-ANSIBLE.md now says the DAC re-clocks only from an idle device, a sink already RUNNING resamples into its current rate, and the section 5 check waits for SUSPENDED first; all four runners and the publish job in a minute and a quarter) and `v2026.09.15.1` (the Sleestak chase the Marshalls: Will and Holly carry the torch, each Sleestak on its own step clock with a four-phase walk, the cornered charge, the crossbow bolt, and the sea creatures in poseidon each on their own stroke; all four runners and the publish job in two and a half minutes) and `v2026.09.15.2` (Athena's olive bough is a real bough, rooted off the left edge, twigs, leaves and olives, swaying in a wind that gusts with the music, and she rides it with the talons hooked under the wood, dancing on top of the sway; the Linux x86_64 job hung in `make scie smoke-scie` a third time while the other three runners finished in under a minute, the 15-minute timeout cancelled it, `gh run rerun --failed` picked the cancelled job up on its own (`gh run rerun --job` refuses once that rerun is in progress) and it took 34 seconds; the publish job followed) and `v2026.09.17` (the light show scales with the terminal's density: shrinking the font used to shrink stars, particles, the wall and every one-dot braille trace, now they keep their size on the glass and gain resolution; the waterfall falls, newest row at the top, and the up/down arrows flip it; rain is blue; the README links Releases and says what the rig is for; all four runners in about a minute and the publish job in a quarter of one, six assets) and `v2026.09.18` (the Stealie redrawn from the 1969 design, traced from a reference image into polygons: the skull's silhouette, the thirteen-point bolt, the eye sockets, nose and teeth, the field blue and red behind the skull, the disc red and blue either side of the bolt, built as mask layers once per terminal size; and the TUI holds the panel awake while the music plays, `KeepAwake` with `xset` and a logind idle inhibitor for up to 90 minutes past the last key, since mpv has no window here to stop the screensaver from; all four runners in under a minute, the slowest the Intel Mac at 50 seconds, and the publish job in 14, six assets) and `v2026.09.19` (the panel is held awake with `xset s reset` on every poll, the way mpv does it on X11, instead of switching the screen saver and DPMS off and putting them back: xfce4-screensaver reads the X state rather than ignoring it, so `xset s off` switched the desktop's screensaver off outright, and a killed player never ran the restore, which is how tiro's panel stopped blanking for a day; nothing is switched off now, so when poseidon is not running the desktop blanks on its own timeout, on its own settings. SIGTERM and SIGHUP become the KeyboardInterrupt the UI loop already treats as quit, so a kill writes the position, stops the light show's parec child instead of orphaning it, and hands the terminal back; all four runners in 81 seconds end to end, the slowest the Intel Mac at 63, the publish job in 9, six assets) and `v2026.09.19.1` (the panel's display found through
logind instead of `$DISPLAY`, so the hold works from tmux on a tty, which is how tiro is
driven; the Linux x86_64 job hung in `make scie smoke-scie` on the first attempt and again
on the rerun, the deadlock above, and there is no release for it) and `v2026.09.19.2` (the
same code plus Poseidon's eyes the colour of the water, cyan pair 1 instead of red, he is
