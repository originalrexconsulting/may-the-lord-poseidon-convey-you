...
scripts/May-The_Lord_Poseidon-Convey-You.py   # (or scripts/deadtui.py) ↵ open, p play (best source on a date: matrix > sbd > aud, on-disk first), ␣ pause,
                       ...
                       # d fetch show in background via gdarchive.py, r resume, s stop, q quit (music stays), Q stop & quit
                       ...
                       # c classical radio, i probe station, v light show (v again cycles its 14 modes, Esc returns)
...
`scripts/gdarchive.py` (stock python3, no pip) searches and downloads from the archive.org Grateful Dead collection into `dead/`. Layout: `dead/shows/<year>/<date>.<identifier>/` for whole shows, `dead/songs/<song>/` for single tracks. Resumable, md5-verified, keeps the taper's txt files and an `archive-metadata.json` per show.
